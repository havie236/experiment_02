...
let currentBlock = 1;
let correctCount = 0; // Đếm số câu đúng trong 1 session (Reset mỗi session)
let currentBlockEarnings = 0; // Tiền kiếm được trong session hiện tại 
let totalEarningsGlobal = 0; // Tổng tiền cả 3 session
...
function updateCorrectUI() { 
    // CHỈ HIỂN THỊ TIỀN CỦA SESSION HIỆN TẠI
...
function startBlock() {
    ...
    // RESET LẠI BỘ ĐẾM VÀ TIỀN MỖI SESSION
    correctCount = 0; 
...
function generateMatrix() {
    ...
    // Chốt số lượng đáp án đúng (30 - 45)
...
function checkAnswer() {
    ...
    if (isCorrect) { 
        ...
        let currentPay = 27 * (42 - correctCount);
        if (currentPay < 0) {
            currentPay = 0;
        }

        currentBlockEarnings += currentPay; 
        totalEarningsGlobal += currentPay;  
        
        updateCorrectUI(); 
        
        // GÕ ĐÚNG KHÔNG HIỆN POPUP NỮA, CỘNG TIỀN RỒI QUA BÀI MỚI LUÔN
    } else {
        // GÕ SAI THÌ VẪN BÁO LỖI
